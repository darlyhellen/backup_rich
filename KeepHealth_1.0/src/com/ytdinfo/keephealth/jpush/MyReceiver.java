...
public class MyReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		// Log.i(TAG, "[MyReceiver]===" + intent.getAction() + "--- extras: " +
		// printBundle(bundle));
		...
		if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			...
			try {
				if (ECDeviceKit.userId != null) {
					...
					if (salId != 0)// 有
					{
						unReadCount = ConversationSqlManager
								.querySessionUnreadCountBySessionId("10000");
						Log.e("Myrecevier", "unReadCount:" + unReadCount);
						ConversationSqlManager.delSession("10000");
					}
					Log.e("Myrecevier", "salId:" + salId);
					ConversationSqlManager.insertSessionRecordV2(msg,
							unReadCount + 1);
				}
			} catch (Exception e) {
			...
			String msg_id = bundle.getString(JPushInterface.EXTRA_MSG_ID);
			DBUtil dbUtil = new DBUtil(MyApp.getInstance());
			TBNews tbNews = dbUtil.queryFirst(msg_id);
			if (tbNews != null && tbNews.getUrl() != null) {
