...
public class ImgInfoSqlManager extends AbstractSQLManager {
	...
	public ImgInfo createImgInfo(String filePath) {
		...
		if((FileUtils.decodeFileLength(filePath) > 204800)
			...
			if(!DemoUtils.createThumbnailFromOrig(filePath, 960, 960, Bitmap.CompressFormat.JPEG, 80, FileAccessor.getImagePathName().getAbsolutePath(), fileNameMD5)) {
				return null;
			}
		...
		if(!DemoUtils.createThumbnailFromOrig(authorityDir +File.separator+ bigFileFullName, 100, 100, Bitmap.CompressFormat.JPEG, 70, authorityDir, thumbName)) {
			return null;
		}
